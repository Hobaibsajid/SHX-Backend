  ...
  ({ strapi }) => ({
    async registerOrganizer(ctx) {
      try {
        ...
        newOrganizer = await strapi.entityService.create(
          "api::organizer.organizer",
          {
            data: {
              firstName: firstName,
    ...
    async updateCoachStatus(ctx) {
      ...
      try {
        const coach = await strapi.entityService.findOne(
          "api::coach.coach",
          coachId,
          {
        ...
        const updatedCoach = await strapi.entityService.update(
          "api::coach.coach",
          coachId,
          {
            data: {
        ...
        const linkedUser = (updatedCoach as any).userId;
    ...
    async addNewEvent(ctx) {
      try {
        const { eventName, startDate, endDate, organizer, venues } =
          ctx.request.body;
    ...
    async addNewMatch(ctx) {
      try {
        ...
        // 2. Fetch event dates
        const eventDetails = await strapi.entityService.findOne(
          "api::event.event",
          event,
          {
        ...
        const eventStartDate = new Date(eventDetails.startDate).setHours(
          0,
          0,
          0,
          0
        );
        const eventEndDate = new Date(eventDetails.endDate).setHours(
          0,
          0,
          0,
          0
        ...
        if (matchDate < eventStartDate || matchDate > eventEndDate) {
          return ctx.send(
            {
              message:
                "Match date must be between the event start and end dates",
            },
            400
          );
        }
        ...
        if (existingMatches.length > 0) {
          return ctx.send(
            {
              message:
                "A match is already scheduled at this venue at the same date and time",
            },
            400
          );
        }
    ...
    async addMatchScore(ctx) {
      try {
        ...
        const existingMatch = await strapi.entityService.findOne(
          "api::match.match",
          matchId
        );
        ...
        // Update the match
        const updatedMatch = await strapi.entityService.update(
          "api::match.match",
          matchId,
          {
            data: {
